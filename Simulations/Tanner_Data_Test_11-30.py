...
# ---------- Samplers ----------
def sample_spline(s, amp, min_ctrl=4,max_ctrl=6):
    """Return smooth profile from random control points in [-amp, amp]."""
    num_ctrl = np.random.randint(min_ctrl,max_ctrl)
    #num_ctrl = 6
...
def sample_gaussians(s, amp, min_bumps=1, max_bumps=3,
                     min_width_frac=0.04, max_width_frac=0.04):
...
def sample_fourier(s, amp, min_modes=1, max_modes=7):
    L = s[-1] - s[0]
...
def generate_random_force_profile(s, amp, method='spline', **kwargs):
    ...
    """
    if method == 'spline':
        return sample_spline(s, amp)
    elif method == 'gaussian':
        return sample_gaussians(s, amp)
    elif method == 'fourier':
        return sample_fourier(s, amp)
    elif method == 'blocks':
        return sample_piecewise(s, amp)
    elif method == 'noise':
        return sample_filtered_noise(s, amp)
    else:
...
EI = 1000.0        # Bending stiffness
GJ = 0.7
r = 5 / np.pi    # Robot radius
N = 16          # Points per segment prolly want to mess with this
...
def objective(L_flat, bending_params, p_target):
    # Unpack parameters needed by bending_3D
    EI, GJ, r, N, m, Base_coord, s_force, f_dist_vec = bending_params
    
    # Reshape L_flat into L1, L2, L3 each of length m
    L1 = L_flat[0:m]
    L2 = L_flat[m:2*m]
    L3 = L_flat[2*m:3*m]
    
    # Run bending_3D with zero external loads (no forces or moments)
    Fx = Fy = Fz = Mx = My = Mz = 0.0
    
    # Run the solver
    y_sol, p_init, _ = bending_3D(
        EI, GJ, r, N, m, L1, L2, L3, Base_coord,
        Fx, Fy, Fz, Mx, My, Mz, s_force, f_dist_vec, ploton=False)
    
    # Compute error metric between computed backbone p_init and target p_def
    # For example, sum of squared distances at all backbone points:
    error = np.linalg.norm(p_init - p_target)
...
def single_simulation(params):
    ...
    fx_profile = f_amp_x * base_profile*0 
    fy_profile = f_amp_y * base_profile*0 
    fz_profile = f_amp_z * base_profile*0 
    ...
    try:
        y_sol, p_def, p_init = bending_3D(
            EI, GJ, r, N, m, L1, L2, L3, Base_coord,
            f_amp_x, f_amp_y, f_amp_z, Mx, My, Mz, s_force, f_dist_data, False)

        #L1_def, L2_def, L3_def = compute_actuator_lengths_from_solution_3D_full(
        #    y_sol, s_force, m, N, r, L1, L2, L3)
        #print(f_amp_x,f_amp_y,f_amp_z)
        #Run bending 3D with zero force until it matches the solution of p_def


        # Now setup parameters for optimizer

        # Example initial guess for actuator lengths (replace with your actual initial guess)
        L1_init = L1
        L2_init = L2
        L3_init = L3

        L_initial = np.hstack([L1_init, L2_init, L3_init])

        # Pack bending_3D parameters you already know
        bending_params = (EI, GJ, r, N, m, Base_coord, s_force, 0*f_dist_data)
        bounds = [(0, 12)] * (3 * m)
        # Run optimizer
        res = minimize(objective, L_initial, args=(bending_params, p_def),
                    method='L-BFGS-B',
                    tol=1e-2,
                    bounds=bounds,              
                    options={'maxiter': 5, 'disp': False})

        # Extract optimized actuator lengths
        L_opt = res.x
        L1_opt = L_opt[0:m]
        L2_opt = L_opt[m:2*m]
        L3_opt = L_opt[2*m:3*m]
        ...
        for i in range(m):
            ...
            result[f'L1_def_{i+1}'] = L1_opt[i]
            result[f'L2_def_{i+1}'] = L2_opt[i]
            result[f'L3_def_{i+1}'] = L3_opt[i]
...
def main():
    ...
    try:
        ...
        np.savez_compressed('simulation_results_partial_11-32.npz', data=results)
    ...
    if len(results) == num_samples:
        np.savez_compressed('simulation_results_all_11-32.npz', data=results)
