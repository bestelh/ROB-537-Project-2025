...
N = 100            # Number of discretization points per segment
...
def profile_sum_gaussians(s, num_bumps, amps, centers, widths):
    s = np.asarray(s)
...
def profile_fourier(s, modes, amps):
    s = np.asarray(s)
    L = s[-1] - s[0]
    prof = np.zeros_like(s)
    k = 1
...
# ---------- Samplers ----------
def sample_spline(s, amp, num_ctrl=8):
...
def sample_gaussians(s, amp, max_bumps=5):
    k = np.random.randint(1, max_bumps+1)
    amps = np.random.uniform(-amp, amp, size=k)
    centers = np.random.uniform(s[0], s[-1], size=k)
    widths = np.random.uniform((s[-1]-s[0])*0.02, (s[-1]-s[0])*0.25, size=k)
    return profile_sum_gaussians(s, k, amps, centers, widths)

def sample_fourier(s, amp, n_modes=5):
    amps = np.random.uniform(-amp, amp, size=n_modes)
    return profile_fourier(s, n_modes, amps)
...
def generate_random_force_profile(s, amp, method='spline', **kwargs):
    ...
    if method == 'spline':
        ...
        return sample_fourier(s, amp, n_modes=kwargs.get('n_modes', 6))
    elif method == 'blocks':
...
GJ = 0.7
r = 5 / np.pi    # Robot radius
N = 50          # Points per segment prolly want to mess with this
...
def single_simulation(params):
    ...
    # Randomly choose force profile types for each axis
    method_x = np.random.choice(['spline','gaussian','fourier','blocks','noise'])
    method_y = np.random.choice(['spline','gaussian','fourier','blocks','noise'])
    method_z = np.random.choice(['spline','gaussian','fourier','blocks','noise'])

    # Generate force distributions
    fx_profile = generate_random_force_profile(s_force, f_amp_x, method=method_x)
    fy_profile = generate_random_force_profile(s_force, f_amp_y, method=method_y)
    fz_profile = generate_random_force_profile(s_force, f_amp_z, method=method_z)

    def normalize_profile(p, amp):
        mx = np.max(np.abs(p))
        return p if mx <= 0 else p * (amp / mx)

    fx_profile = normalize_profile(fx_profile, f_amp_x)
    fy_profile = normalize_profile(fy_profile, f_amp_y)
    fz_profile = normalize_profile(fz_profile, f_amp_z)
...
def generate_random_params(num_samples):
    ...
    for _ in range(num_samples):
        ...
        f_amp_x = np.random.uniform(force_min, force_max)
        f_amp_y = np.random.uniform(force_min, force_max)
        f_amp_z = np.random.uniform(force_min, force_max)
...
def main():
    ...
    results = []  # metadata and outputs (dicts)
    max_workers = 12  # Adjust based on your CPU threads
    ...
    # ---- Save everything ----
    np.savez_compressed('simulation_results_all_2.npz', data=results)

    print("Saved all results to 'simulation_results_all_2.npz'")
    print("Each element contains:")
    print("  • Input actuator lengths (L1, L2, L3)")
    print("  • Force amplitudes & generation methods")
    print("  • Deformed actuator lengths")
    print("  • Full distributed force profiles (fx, fy, fz, s_force)")
