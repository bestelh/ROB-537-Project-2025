"""
Neural Network Model Evaluation Script for November 19th Dataset

This script loads a pre-trained neural network model and evaluates it on test data.
It can load models saved by the Feed_forward_nn_training_v2.py script and provides
detailed performance analysis including predictions, error statistics, and visualizations.

Modified for the November 19th dataset with 192-point force distributions (576 total outputs).
...
class ModelEvaluator:
    ...
    def load_model(self, model_path):
        ...
        try:
            ...
            # Load output layer weights (new dynamic architecture)
            if 'w_out' in self.model_data:
                self.w_out = self.model_data['w_out']
                self.b_out = self.model_data['b_out']
                self.dynamic_architecture = True
            else:
                # Legacy architecture - load w2/w3/w4 as before
            ...
            # Load all hidden layer weights dynamically (for new architecture)
            if self.dynamic_architecture:
                self.hidden_weights = {}
                self.hidden_biases = {}
                for layer_num in range(2, 16):  # Check up to 15 hidden layers
                    w_name = f'w{layer_num}'
                    b_name = f'b{layer_num}'
                    if w_name in self.model_data:
                        self.hidden_weights[layer_num] = self.model_data[w_name]
                        self.hidden_biases[layer_num] = self.model_data[b_name]
            ...
            if self.dynamic_architecture:
                arch_str = f"  Architecture: {self.network_config['input_size']}"
                for layer_num in range(1, self.num_hidden_layers + 1):
                    hidden_size = self.network_config.get(f'hidden_size{layer_num}', 'Unknown')
                    arch_str += f" → {hidden_size}"
                arch_str += f" → {self.network_config['output_size']}"
                print(arch_str)
            else:
    ...
    def forward_pass(self, x):
        ...
        Supports both legacy and dynamic architectures.
        ...
        if self.dynamic_architecture:
            ...
            for layer_num in range(2, self.num_hidden_layers + 1):
                if layer_num in self.hidden_weights:
                    z = self.hidden_weights[layer_num] @ current_input + self.hidden_biases[layer_num]
                    current_input = self.tanh_activation(z)
                else:
                    print(f"Warning: Missing weights for layer {layer_num}")
                    break
            
            # Output layer
            z_out = self.w_out @ current_input + self.b_out
            output = self.linear_activation(z_out)
            return output
        ...
        print(f"Error: Unsupported architecture with {self.num_hidden_layers} layers")
        return None
    ...
    def load_test_data(self, npz_file_path="simulation_results_all_11-19.npz", train_split=0.5):
        """
        Load and prepare test data (same as training script).
        Modified for November 19th dataset with 192-point force distributions.
        ...
        for sim in simulation_results:
            ...
            # Concatenated force distributions (192 points each for November 19th dataset)
        ...
        y_test_norm = (y_test_raw.T - self.y_mean) / self.y_std  # Shape: (576, n_test)
    ...
    def evaluate_model(self, x_test_norm, y_test_norm, x_test_raw=None, y_test_raw=None):
        ...
        # Component-wise analysis (Fx, Fy, Fz) - Updated for 192-point distributions
        fx_error = error_real[:, :192] if len(error_real.shape) > 1 else error_real[:192]
        fy_error = error_real[:, 192:384] if len(error_real.shape) > 1 else error_real[192:384]
        fz_error = error_real[:, 384:576] if len(error_real.shape) > 1 else error_real[384:576]
    ...
    def plot_evaluation_results(self, results, sample_indices=None, save_plots=True):
        ...
        for i, (comp, color) in enumerate(zip(components, colors)):
            ...
            # Updated for 192-point distributions
            start_idx = i * 192
            end_idx = (i + 1) * 192
        ...
        if save_plots:
            plt.savefig('model_evaluation_results_11-19.png', dpi=300, bbox_inches='tight')
            print("Plots saved to 'model_evaluation_results_11-19.png'")
...
def find_model_files():
    ...
    # Search in current directory and parent Simulations directory
    search_dirs = [Path.cwd(), Path.cwd().parent]
    
    for search_dir in search_dirs:
        if search_dir.exists():
            # Look for .npz files that might be models (prioritize 11-19 models)
            for pattern in ["*_11-19.npz", "*11-19*.npz", "*.npz"]:
                for file_path in search_dir.glob(pattern):
                    # Exclude data files
                    if "simulation_results" not in file_path.name:
...
def main():
    ...
    print("Neural Network Model Evaluation - November 19th Dataset")
    print("="*60)
    ...
    if not model_files:
        ...
        print("Expected files: trained_model_single_11-19.npz, trained_model_best_11-19.npz")
    ...
    try:
        ...
        sample_indices = [401]  # Different samples to evaluate
        evaluator.plot_evaluation_results(results, sample_indices=sample_indices)
        
        print("\nEvaluation complete!")
        print("Check 'model_evaluation_results_11-19.png' for detailed plots.")
