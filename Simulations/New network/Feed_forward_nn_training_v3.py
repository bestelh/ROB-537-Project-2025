...
def data_import(npz_file_path="simulation_results_all_11-19.npz", train_split=0.5):
    ...
    for sim in simulation_results:
        # Input: Difference between input and deformed for each actuator (12 features total)
        # For each segment: (L1_input - L1_def), (L2_input - L2_def), (L3_input - L3_def)
        inputs = [
    ...
    X = np.array(input_features)  # Shape: (n_samples, 12)
    ...
    x_train = X[:split_idx].T  # Shape: (12, n_train) - First 50% for training
    y_train = Y[:split_idx].T  # Shape: (576, n_train)
    x_test = X[split_idx:].T   # Shape: (12, n_test) - Last 50% for testing
...
class FeedForwardNN:
    ...
    def __init__(self, input_size=12, hidden_size1=768, hidden_size2=512, hidden_size3=256, hidden_size4=128, hidden_size5=64,
...
def run_single_training():
    ...
    try:
        ...
        network = FeedForwardNN(
            input_size=12, hidden_size1=300, hidden_size2=200, hidden_size3=100, 
...
def run_multiple_training(num_runs=10):
    ...
    try:
        ...
        for run in range(num_runs):
            ...
            network = FeedForwardNN(
                input_size=12, hidden_size1=1024, output_size=576,
