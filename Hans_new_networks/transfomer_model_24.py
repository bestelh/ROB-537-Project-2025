...
class DeformationToForceTransformer(nn.Module):
    ...
    Output: (batch_size, 24, 3) - 24 position tokens with [fx, fy, fz]
    """
    
    def __init__(self, 
                 input_dim=3,           # Features per actuator token
                 output_dim=3,          # Force components per position
                 output_seq_len=24,     # Output sequence length (force positions)
    ...
    def forward(self, src):
        ...
        Returns:
            (batch_size, 24, 3) - predicted distributed forces
        """
        ...
        position_queries = self.position_queries.unsqueeze(0).repeat(batch_size, 1, 1)  # (batch_size, 24, d_model)
        ...
        output = self.output_proj(output)  # (batch_size, 24, 3)
...
class ForceEstimatorTrainer:
    ...
    def train_model(self, x_train, y_train, 
                   ...
                   patience=15,
                   gradient_clip=1.0,
                   save_path='best_transformer_24x3.pth'):
        """
        Train the transformer model
        
        Args:
            x_train: (n_samples, 12, 3) - actuator data
            y_train: (n_samples, 24, 3) - force data
    ...
    def save_model_with_config(self, save_path='complete_transformer_model_24x3.pth'):
        # Ensure save directory exists
        save_dir = Path(save_path).parent
        save_dir.mkdir(parents=True, exist_ok=True)
...
def load_training_data(npz_file_path="12_1_training_hans44.npz"):
    """
    Load training data from .npz file
    
    Returns:
        x_train: (n_samples, 12, 3) - 12 actuator tokens with [initial, deformed, difference]
        y_train: (n_samples, 24, 3) - 24 position tokens with [fx, fy, fz]
    """
    from pathlib import Path
...
if __name__ == "__main__":
    ...
    # Load training data
    x_train, y_train, actual_output_len = load_training_data("12_1_training_hans44.npz")
    ...
    save_path = Path(__file__).parent / 'deformation_to_force_transformer_24x3.pth'
    ...
    complete_save_path = Path(__file__).parent / 'complete_transformer_model_24x3.pth'
    ...
    with torch.no_grad():
        ...
        pred_forces = model(x_sample).cpu().numpy()[0]  # (24, 3)
        true_forces = y_train[sample_idx]  # (24, 3)
